...
uint32_t read_mcycleh() {
    uint32_t cyclesh;
    asm volatile ("csrr %0, mcycleh" : "=r"(cyclesh));
    return cyclesh;
}

uint32_t elapsed_time(uint32_t start, uint32_t starth, uint32_t stop, uint32_t stoph) {
    uint32_t res, resh, helperh, helper;
    
    /*if(stop>=start){ //todo está bien
        res = stop-start;
        resh= stoph-starth;
    }
    else{
...
int main(void) {
    ...
    kprintf("\nDONE! FGPU execution: %d ns \n", elapsed_time(tstart, tstarth, tstop, tstoph));
